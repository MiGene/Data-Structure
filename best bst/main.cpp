...
void recur(int k, int m, CP::map_bst<int,int> &bst) {
    //std::cout << "in ";
    int per;
    if (m==k) {
        per = k;
    ...
    /*if (((k+per*(1/2)) - (k-per*(1/2)))==2 || ((k+per*(1/2)) - (k-per*(1/2)))==2) {
        return ;
    }
    else {*/
        int a = k-(per/2);
...
void gen_best_bst(int n,CP::map_bst<int,int> &bst) {
  ...
  //ครึ่งของครึ่งเสมอ insert ตัวที่ 1/2(1/4 ) กับ 1/2(3/4)  -> 1/2(1/2(1/4))
  //มีเคสที่ n=0

    int middle = n/2 +1;
    bst.insert({middle,0});
    recur(middle, middle, bst);
